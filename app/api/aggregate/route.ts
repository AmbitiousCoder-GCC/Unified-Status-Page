...
export async function GET(request: NextRequest) {
  try {
    // Using the optimized 'sql' tag from @vercel/postgres which is pre-configured for Vercel environments.
    ...
    if (!rows || rows.length === 0) {
      return NextResponse.json(VENDORS_LIST.map(v => ({
        vendorId: v.id,
        fetchedAt: new Date().toISOString(),
        overallStatus: "unknown",
        statusDescription: "Initializing database...",
        uptimePct15d: 100,
        uptimeHistory: [],
        activeIncidents: [],
        pastIncidents: [],
    ...
    const statuses: VendorStatus[] = rows.map((row: any) => {
      ...
      return {
        ...
        statusDescription: row.description || "Vendor operational",
        uptimePct15d,
    ...
    console.error("[Aggregate] Fatal:", error);
