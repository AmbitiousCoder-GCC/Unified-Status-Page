...
/**
 * Adapter for Auth0 Status API
 * @see https://status.auth0.com/api/v2/status.json
 */
export const Auth0Adapter = {
    ...
    async fetchStatus(): Promise<VendorStatus> {
        try {
            const response = await fetch("https://auth0.statuspage.io/api/v2/summary.json", {
            ...
            console.error("Auth0 fetch failed:", error);
            return {
                vendor_id: this.id,
                status: "OPERATIONAL",
