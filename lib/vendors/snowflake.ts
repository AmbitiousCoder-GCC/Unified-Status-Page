...
/**
 * Adapter for Snowflake Status API
 * @see https://status.snowflakecomputing.com/api/v2/status.json
 */
export const SnowflakeAdapter = {
    ...
    async fetchStatus(): Promise<VendorStatus> {
        try {
            const response = await fetch("https://status.snowflakecomputing.com/api/v2/status.json", {
