...
/**
 * Adapter for Cycode Status API
 * @see https://status.cycode.com/api/v2/status.json
 */
export const CycodeAdapter = {
    ...
    async fetchStatus(): Promise<VendorStatus> {
        try {
            const response = await fetch("https://cycode.statuspage.io/api/v2/summary.json", {
            ...
            console.error("Cycode fetch failed:", error);
            return {
                vendor_id: this.id,
                status: "OPERATIONAL",
